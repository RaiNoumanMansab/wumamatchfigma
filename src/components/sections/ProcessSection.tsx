...
export const ProcessSection: React.FC = () => {
  ...
  return (
    <section
      id="process"
      className="bg-[#071415]  py-16 sm:py-20 px-6 sm:px-8 lg:px-12 relative overflow-hidden"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
          {steps.map((step, idx) => (
            <motion.div
              ...
              className="min-h-[300px] lg:min-h-[380px] bg-transparent  border border-white/18 hover:border-[#0F9598]/70 rounded-[10px] p-6 sm:p-7 lg:p-8 text-left transition-all duration-500 flex flex-col group relative overflow-hidden shadow-[0_18px_60px_rgba(28,27,25,0.35)] hover:shadow-[0_26px_80px_rgba(15,149,152,0.16)]"

            >
              <div className="absolute inset-0 bg-[#0F9598]/0 group-hover:bg-[#0F9598]/[0.035] transition-colors duration-500 pointer-events-none" />

              <div className="relative z-10 flex items-start justify-between">
                <span className="font-sans text-sm sm:text-[15px] font-light text-white [line-height:1.35]">
                  {step.stepNum}
                </span>
                {/* <span className="font-sans text-xl text-white leading-none group-hover:text-[#0F9598] transition-colors duration-300">
                  +
                </span> */}
<ArrowRight
  className="w-4 h-4 stroke-[2.3] text-white rotate-90 md:rotate-0 transition-transform duration-300 group-hover:text-[#0F9598]"
/>
