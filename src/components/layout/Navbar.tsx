...
export const Navbar: React.FC = () => {
  ...
  const currentLanguageIndex = languageOptions.findIndex((option) => option.locale === locale);
  const currentLanguage = languageOptions[currentLanguageIndex] ?? languageOptions[0];
  const nextLanguage = languageOptions[(currentLanguageIndex + 1) % languageOptions.length];
  ...
  return (
    ...
    >
      <nav
        className="max-w-7xl mx-auto bg-[#F9F8F4]/80 backdrop-blur-md border border-white/20 rounded-[10px] shadow-luxury px-4 sm:px-6 py-2.5 flex items-center justify-between pointer-events-auto transition-all duration-300"
      >
        
        <a href="#home" className="relative flex items-center group rounded-[10px] border border-[#9B7A5B]/35
px-2 py-1 transition-colors duration-300 hover:border-[#0F9598]/45">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className="h-12 w-12 bg-white text-[#0F9598] border border-white/80 flex items-center justify-center rounded-[10px] shadow-sm transition-colors duration-300 group-hover:bg-[#F9F8F4] group-hover:text-[#1C1B19]">
              <motion.span
                variants={{ rest: { rotate: 0 }, hover: { rotate: 50, transition: { duration: 0.42, ease: [0.22, 1, 0.36, 1] } } }}
